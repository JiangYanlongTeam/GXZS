...
<script>
    try {
        if(document.cookie.indexOf('logincookiecheck=<%=testcookie%>')==-1) {
            ...
            <%if(loginid.equalsIgnoreCase("sysadmin") || !isExist(request,response)){%>
            document.location.href = '/wui/main.jsp';
            <%}else{%>
            document.location.href = '/cloudstore/app/no0000155/fdjt/index.jsp';
            <%}%>
        }
    }
    catch(e){
...
<%!
    public boolean isExist(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            recordSet.execute("select * from uf_kjry_dt1 where depid = '"+depid+"' and sfkj = '0'");
            if(recordSet.next()) {
                return true;
            }
            recordSet.execute("select * from uf_kjry_dt1 where (hrmids like '%"+userid+"%' or hrmids like '%"+userid+",%' or hrmids like '%,"+userid+",%' or hrmids like '%,"+userid+"%') and sfkj = '0'");
            if(recordSet.next()) {
                return true;
            }
            return false;
