...
<script language="javascript">
    ...
    jQuery(document).ready(function () {
        ...
        if ('<%=nodetype%>' == '0') {//for travel expense,apply node
            ...
            if (jQuery("#field<%=gysmcid%>").is(":hidden")) {
                ...
                jQuery("#field<%=gysmcid%>span").html("<img src='/wui/theme/ecology8/skins/default/general/browser_wev8.png' style='cursor:pointer;' onclick=\"getdata()\" />");
            }
            ...
            if (jQuery("#field<%=lhhid%>").is(":hidden")) {
                ...
                jQuery("#field<%=lhhid%>span").html("<img src='/wui/theme/ecology8/skins/default/general/browser_wev8.png' style='cursor:pointer;' onclick=\"getdata3()\" />");
            }
    ...
    checkCustomize = function () {
        ...
        var dszfje_value = jQuery("#field<%=dszfje%>").val();
        var dgzfje_value = jQuery("#field<%=dgzfje%>").val();
        var dgzffs_value = jQuery("#field<%=dgzffs%>").val();
        var khhmc_value = jQuery("#field<%=khhmc_value%>").val();
        ...
        var sum = parseFloat(dgzfje_value) + parseFloat(dszfje_value);
        var zflb_value = jQuery("#field<%=zflbid%>").val();
        if (zflb_value == "0" || zflb_value == "2") {
            if (gysmc_value == "") {
                Dialog.alert("供应商名称必填！");
                return false;
            }
            if (lhh_value == "") {
                Dialog.alert("联行号必填！");
            ...
            if (khhmc_value == "") {
                Dialog.alert("开户行名称必填！");
                return false;
            }
            if (yhzhid_value == "") {
                Dialog.alert("供应商银行账号必填！");
                return false;
            }
        }
        if (zflb_value == "3" && sbje_value != 0) {
        ...
        if (sum != parseFloat(sbje_value) && zflb_value == "0") {
            Dialog.alert("对私支付金额和对公支付金额合计必须等于实报金额");
            return false;
        }
        if (zflb_value == "1" && (parseFloat(sbje_value) != parseFloat(dszfje_value))) {
            Dialog.alert("对私支付金额需等于实报金额");
            return false;
        }
        if ((zflb_value == "4" || zflb_value == "2") && (parseFloat(sbje_value) != parseFloat(dgzfje_value))) {
            Dialog.alert("对公支付金额需等于实报金额");
            return false;
        }
        return true;
    };
