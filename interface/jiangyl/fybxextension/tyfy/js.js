    ...
    jQuery(function(){
        jQuery("#field49448").bindPropertyChange(function(){
            ...
            if(parseInt(selectvalue) == 2) { //直接报销，第二个明细不能填写
                jQuery("button[name=\"addbutton1\"]").attr("onclick","");
            }  else if(parseInt(selectvalue) == 0){//第二个明细填一条
                jQuery("button[name=\"addbutton1\"]").attr("onclick",addRow1(1));
                cxjk_kz();
        ...
        //清空明细表1
        function clear(){
            var detailnum2 = jQuery("#indexnum1").val();
            if(detailnum2 > 0) {
                for(var i = 0; i < parseInt(detailnum2); i++) {
                    if(jQuery("input[name=\"check_node_1\"][value=\""+i+"\"]")){
                        jQuery("input[name=\"check_node_1\"][value=\""+i+"\"]").attr("checked","checked");
                    }
                }
                deleteRow1(1);
                jQuery("#indexnum1").val("0");
            }
            jQuery("button[name=\"addbutton1\"]").attr("onclick","");
        }
