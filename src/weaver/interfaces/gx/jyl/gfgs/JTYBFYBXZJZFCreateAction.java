 ...
 * 股份公司-通用费用报销-资金支付
 */
public class JTYBFYBXZJZFCreateAction extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		String zs_column = "zjzfdfjzs"; //附件张数
		String zs_value = "";
		// 付款方行号
		...
		if (("2".equals(bxfs_value) || ("0".equals(bxfs_value) && Double.parseDouble(sqje_value) > 0))) {
			...
			writeLog("股份公司-通用费用报销-资金支付傳入xml参数：" + xmlstring);
			...
			try {
				...
				writeLog("股份公司-通用费用报销-资金支付返回消息：" + returnmessage);
				...
				if (null != map && !map.isEmpty()) {
					...
					if ("S".equalsIgnoreCase(type)) {
						publicmethod.updateJTYBFYBXZJZFD(requestid, type, code, message, no, "GFGS_YBFYBX");
					} else {
						publicmethod.setFailMessage(request, "failed", "股份公司-通用费用报销-资金支付失败：TYPE：" + type + " code："
								+ code + " applyno：" + no + " message：" + message);
						return SUCCESS;
					}
				}
			} catch (RemoteException e) {
				publicmethod.setFailMessage(request, "failed", "股份公司-通用费用报销-资金支付接口异常：" + e);
				return SUCCESS;
			}
		}
		return SUCCESS;
