...
import weaver.interfaces.gx.jyl.gfgs.model.JTCLFBXSFModel;
 ...
 * 股份公司-差旅费报销释放
 */
public class JTCLFBXSQSFAction extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		if (zflb_value.equals("1")) {// 对私
			String xmlstring = "";
			JTCLFBXSFModel model = new JTCLFBXSFModel(gsdm_value, zjpzbh_value, "Y", dszfje_value);
			try {
				xmlstring = XMLUtil.beanToXml(model, JTCLFBXSFModel.class);
			} catch (JAXBException e) {
				e.printStackTrace();
			}

			writeLog("股份公司-差旅费报销释放（对私）传入xml参数：" + xmlstring);
			...
			try {
				...
				writeLog("股份公司-差旅费报销释放（对私）返回消息：" + returnmessage);
				...
				if (null != map && !map.isEmpty()) {
					...
					if (!"E".equalsIgnoreCase(type)) {
						publicmethod.updateJTCLFBXSFDS(requestid, type, code, message, "GFGS_CLFYBX");
					} else {
						publicmethod.setFailMessage(request, "failed",
								"股份公司-差旅费报销释放（对私）失败：TYPE：" + type + " code：" + code + " message：" + message);
						return SUCCESS;
					}
				}
			} catch (RemoteException e) {
				publicmethod.setFailMessage(request, "failed", "股份公司-差旅费报销释放（对私）接口异常：" + e);
				return SUCCESS;
			}
		}
		if (zflb_value.equals("2")) {// 对公
			String xmlstring = "";
			JTCLFBXSFModel model = new JTCLFBXSFModel(gsdm_value, zjpzbh1_value, "Y", dgzfje_value);
			try {
				xmlstring = XMLUtil.beanToXml(model, JTCLFBXSFModel.class);
			} catch (JAXBException e) {
				e.printStackTrace();
			...
			writeLog("股份公司-差旅费报销释放（对公）传入xml参数：" + xmlstring);
			...
			try {
				DT_1029_ALL2ERP_ZJYSSF_RETURN = proxy.SI_1029_ALL2ERP_ZJYSSF_OUT(DT_1029_ALL2ERP_ZJYSSF);
				String returnmessage = DT_1029_ALL2ERP_ZJYSSF_RETURN.getINPUT();
				writeLog("股份公司-差旅费报销释放（对公）返回消息：" + returnmessage);
				Map<String, String> map = publicmethod.readXMLForSF(returnmessage);
				if (null != map && !map.isEmpty()) {
					String type = (String) map.get("TYPE");
					String code = (String) map.get("CODE");
					String message = (String) map.get("MESSAGE");
					if (!"E".equalsIgnoreCase(type)) {
						publicmethod.updateJTCLFBXSFDG(requestid, type, code, message, "GFGS_CLFYBX");
					} else {
						publicmethod.setFailMessage(request, "failed",
								"股份公司-差旅费报销释放（对公）失败：TYPE：" + type + " code：" + code + " message：" + message);
						return SUCCESS;
					}
				}
			} catch (RemoteException e) {
				publicmethod.setFailMessage(request, "failed", "股份公司-差旅费报销释放（对公）接口异常：" + e);
				return SUCCESS;
			}
		}
		if (zflb_value.equals("0")) {// 对私 对公
			...
			String xmlstring1 = "";
			JTCLFBXSFModel model1 = new JTCLFBXSFModel(gsdm_value, zjpzbh1_value, "Y", dgzfje_value);
			try {
				xmlstring1 = XMLUtil.beanToXml(model1, JTCLFBXSFModel.class);
			} catch (JAXBException e) {
				e.printStackTrace();
			}

			writeLog("股份公司-差旅费报销释放（对公）传入xml参数：" + xmlstring1);
