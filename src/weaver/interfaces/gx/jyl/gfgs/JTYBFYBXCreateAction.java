...
public class JTYBFYBXCreateAction extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		String splx = getPropValue("GXBX", "GFGS_YBFYBX_FYSPLX");
			...
			if ("0".equals(zflb_value)) {
				...
				while (rs.next()) {
					...
					if ("0".equals(sfzp_value)) {
						...
						String AUFNR_VALUES = "";
						if (splx.equals(fylx_value)) {
							AUFNR_VALUES = cph_value;
						} else {
							AUFNR_VALUES = gsxm_value;
						}
						// 稅不带成本中心
						JTCLFBXCreate_ItemModel line2 = new JTCLFBXCreate_ItemModel("S", jxskm_value, "", "",
				...
				writeLog("股份公司-一般费用报销(直接报销／个人)传入xml参数：" + xmlstring);
				...
				try {
					...
					writeLog("股份公司-一般费用报销(直接报销／个人)创建返回消息：" + returnmessage);
					...
					if (null != map && !map.isEmpty()) {
						...
						if ("S".equalsIgnoreCase(ZSTATU)) {
							publicmethod.updateJTYBFBXCJ(requestid, RET_ACCNO, ZSTATU, MESSAGE, pzacdocno, "GFGS_YBFYBX");
						} else {
							publicmethod.setFailMessage(request, "failed",
									"股份公司-一般费用报销(直接报销／个人)创建失败：RET_ACCNO：" + RET_ACCNO + " ZSTATU：" + ZSTATU + " ZDESC：" + MESSAGE);
							return SUCCESS;
						}
					}
				} catch (RemoteException e) {
					publicmethod.setFailMessage(request, "failed", "股份公司-一般费用报销(直接报销／个人)创建接口异常：" + e);
					return SUCCESS;
				}
			}
			//  对公
			if ("1".equals(zflb_value) || "3".equals(zflb_value)) {
				String xmlstring = "";
				List<JTCLFBXCreate_HeadModel> headlist = new ArrayList<JTCLFBXCreate_HeadModel>();
				JTCLFBXCreate_HeadModel headermodel = new JTCLFBXCreate_HeadModel(gsdm_value, pzlx_value, pzrq_value,
						gzrq_value, headZY, ckpzbh_value, hbm_value, "", pzzdr_value, "OA系统",fjzs_value);
				...
				writeLog("股份公司-一般费用报销(直接报销／对公)传入xml参数：" + xmlstring);
				...
				try {
					DT_1072_ALL2ERP_KJPZ_RETURN = proxy.SI_1072_ALL2ERP_KJPZ_OUT(DT_1072_ALL2ERP_KJPZ);
					String returnmessage = DT_1072_ALL2ERP_KJPZ_RETURN.getINPUT();
					writeLog("股份公司-一般费用报销(直接报销／对公)创建返回消息：" + returnmessage);
					Map<String, String> map = publicmethod.readXMLForCLFBXSQAA(returnmessage);
					if (null != map && !map.isEmpty()) {
						String RET_ACCNO = (String) map.get("RET_ACCNO");
						String ZSTATU = (String) map.get("MTYPE");
						...
						if ("S".equalsIgnoreCase(ZSTATU)) {
							...
							publicmethod.setFailMessage(request, "failed",
									"股份公司-一般费用报销(直接报销／对公)创建失败：RET_ACCNO：" + RET_ACCNO + " ZSTATU：" + ZSTATU + " ZDESC：" + MESSAGE);
							return SUCCESS;
						}
					}
				} catch (RemoteException e) {
					publicmethod.setFailMessage(request, "failed", "股份公司-一般费用报销(直接报销／对公)创建接口异常：" + e);
					return SUCCESS;
				}
			}
		}
		...
		if ("0".equals(bxfs_value) && "0".equals(sbje_value)) {
			...
			writeLog("股份公司-一般费用报销(冲销借款/个人 实报金额等于0)传入xml参数：" + xmlstring);
			...
			try {
				DT_1072_ALL2ERP_KJPZ_RETURN = proxy.SI_1072_ALL2ERP_KJPZ_OUT(DT_1072_ALL2ERP_KJPZ);
				String returnmessage = DT_1072_ALL2ERP_KJPZ_RETURN.getINPUT();
				writeLog("股份公司-一般费用报销(冲销借款/个人 实报金额等于0)创建返回消息：" + returnmessage);
				Map<String, String> map = publicmethod.readXMLForCLFBXSQAA(returnmessage);
				if (null != map && !map.isEmpty()) {
					String RET_ACCNO = (String) map.get("RET_ACCNO");
					String ZSTATU = (String) map.get("MTYPE");
					...
					if ("S".equalsIgnoreCase(ZSTATU)) {
						...
						publicmethod.setFailMessage(request, "failed",
								"股份公司-一般费用报销(冲销借款/个人)创建失败：RET_ACCNO：" + RET_ACCNO + " ZSTATU：" + ZSTATU + " ZDESC：" + MESSAGE);
						return SUCCESS;
					}
				}
			} catch (RemoteException e) {
				publicmethod.setFailMessage(request, "failed", "股份公司-一般费用报销(冲销借款/个人)创建接口异常：" + e);
				return SUCCESS;
			}
		}
		if ("0".equals(bxfs_value) && Double.parseDouble(sbje_value) > 0) {
			...
			lines.add(line2);
			JTCLFBXCreate_ItemModel line3 = new JTCLFBXCreate_ItemModel("H", qtyfkgr_value, ygbh_value, "", "",
			...
			writeLog("股份公司-一般费用报销(冲销借款／个人 实报金额大于0)传入xml参数：" + xmlstring);
			...
			try {
				DT_1072_ALL2ERP_KJPZ_RETURN = proxy.SI_1072_ALL2ERP_KJPZ_OUT(DT_1072_ALL2ERP_KJPZ);
				String returnmessage = DT_1072_ALL2ERP_KJPZ_RETURN.getINPUT();
				writeLog("股份公司-一般费用报销(冲销借款／个人 实报金额大于0)创建返回消息：" + returnmessage);
				Map<String, String> map = publicmethod.readXMLForCLFBXSQAA(returnmessage);
				if (null != map && !map.isEmpty()) {
					String RET_ACCNO = (String) map.get("RET_ACCNO");
					String ZSTATU = (String) map.get("MTYPE");
