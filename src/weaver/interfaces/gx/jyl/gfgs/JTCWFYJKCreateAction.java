...
import weaver.interfaces.gx.jyl.gfgs.model.JTCWFYJKCreateModel;
import weaver.interfaces.gx.jyl.gfgs.model.JTCWFYJKCreate_HeadModel;
 ...
 * 股份公司-费用借款-创建
 */
public class JTCWFYJKCreateAction extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		if (!"submit".equals(src)) {
			new weaver.general.BaseBean().writeLog("股份公司-费用借款-创建退回操作，不执行接口.");
			return SUCCESS;
		}
		// 单据类型-值
		String djlx_value = "";
		...
		List<JTCWFYJKCreate_HeadModel> hEAD = new ArrayList<JTCWFYJKCreate_HeadModel>();
		JTCWFYJKCreate_HeadModel model = new JTCWFYJKCreate_HeadModel(djlx_value, gsdm_value, cn_value, kjqj_value, cbzx_value, yssqh_value,
		...
		hEAD.add(model);
		JTCWFYJKCreateModel header = new JTCWFYJKCreateModel(hEAD);
		try {
			xmlstring = XMLUtil.beanToXml(header, JTCWFYJKCreateModel.class);
		} catch (JAXBException e) {
			e.printStackTrace();
		}
		
		writeLog("股份公司-费用借款-创建傳入xml参数：" + xmlstring);
		...
		try {
			...
			writeLog("股份公司-费用借款-创建返回消息：" + returnmessage);
			...
			if (null != map && !map.isEmpty()) {
				...
				if ("S".equalsIgnoreCase(type)) {
					...
					publicmethod.setFailMessage(request, "failed",
							"股份公司-费用借款-创建失败：TYPE：" + type + " code：" + code
									+ " applyno：" + no + " message：" + message);
					return SUCCESS;
				}
			}
		} catch (RemoteException e) {
			publicmethod.setFailMessage(request, "failed", "股份公司-费用借款-创建接口异常："
					+ e);
			return SUCCESS;
		}
		return SUCCESS;
