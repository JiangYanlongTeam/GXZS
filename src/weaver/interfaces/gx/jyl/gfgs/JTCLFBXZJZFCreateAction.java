...
public class JTCLFBXZJZFCreateAction extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		if (!"submit".equals(src)) {
			new weaver.general.BaseBean().writeLog("股份公司-差旅费用报销-资金支付-创建退回操作，不执行接口.");
			return SUCCESS;
		}
		// 单据类型-值
		String djlx_value = "";
		...
		if (("1".equals(bxfs_value) || ("0".equals(bxfs_value) && Double.parseDouble(sqje_value) > 0))) {
			...
			if (zflb_value.equals("1")) {// 对私
				...
				writeLog("股份公司-差旅费用报销-资金支付（对私）傳入xml参数：" + xmlstring);
				...
				try {
					...
					writeLog("股份公司-差旅费用报销-资金支付（对私）返回消息：" + returnmessage);
					...
					if (null != map && !map.isEmpty()) {
						...
						if ("S".equalsIgnoreCase(type)) {
							publicmethod.updateJTCLFXBXZJZFDDS(requestid, type, code, message, no, "GFGS_CLFYBX");
						} else {
							publicmethod.setFailMessage(request, "failed", "股份公司-差旅费用报销-资金支付（对私）失败：TYPE：" + type
									+ " code：" + code + " applyno：" + no + " message：" + message);
							return SUCCESS;
						}
					}
				} catch (RemoteException e) {
					publicmethod.setFailMessage(request, "failed", "股份公司-差旅费用报销-资金支付（对私）接口异常：" + e);
					return SUCCESS;
				}
			}
			if (zflb_value.equals("2")) {// 对公
				...
				writeLog("股份公司-差旅费用报销-资金支付（对公）傳入xml参数：" + xmlstring);
				SI_1049_ALL2ERP_ZJZFSQ_OUTProxy proxy = new SI_1049_ALL2ERP_ZJZFSQ_OUTProxy();
				DT_1049_ALL2ERP_ZJZFSQ DT_1049_ALL2ERP_ZJZFSQ = new DT_1049_ALL2ERP_ZJZFSQ();
				DT_1049_ALL2ERP_ZJZFSQ.setOUTPUT(xmlstring);
				DT_1049_ALL2ERP_ZJZFSQ.setSYSTEM(xtzd_value);
				...
				try {
					DT_1049_ALL2ERP_ZJZFSQ_RETURN = proxy.SI_1049_ALL2ERP_ZJZFSQ_OUT(DT_1049_ALL2ERP_ZJZFSQ);
					String returnmessage = DT_1049_ALL2ERP_ZJZFSQ_RETURN.getINPUT();
					writeLog("股份公司-差旅费用报销-资金支付（对公）返回消息：" + returnmessage);
					Map<String, String> map = publicmethod.readXMLForSQ(returnmessage);
					if (null != map && !map.isEmpty()) {
						String type = (String) map.get("TYPE");
						String no = (String) map.get("APPLYNO");
						...
						String message = (String) map.get("MESSAGE");
						if ("S".equalsIgnoreCase(type)) {
							publicmethod.updateJTCLFXBXZJZFDDG(requestid, type, code, message, no, "GFGS_CLFYBX");
						} else {
							publicmethod.setFailMessage(request, "failed", "股份公司-差旅费用报销-资金支付（对公）失败：TYPE：" + type
									+ " code：" + code + " applyno：" + no + " message：" + message);
							return SUCCESS;
						}
					}
				} catch (RemoteException e) {
					publicmethod.setFailMessage(request, "failed", "股份公司-差旅费用报销-资金支付（对公）接口异常：" + e);
					return SUCCESS;
				}
			}
			if (zflb_value.equals("0")) {// 对私 对公
				...
				List<JTCLFBXZJZFCreate_HeadModel> hEAD1 = new ArrayList<JTCLFBXZJZFCreate_HeadModel>();
				JTCLFBXZJZFCreate_HeadModel model1 = new JTCLFBXZJZFCreate_HeadModel(djlx_value, gsdm_value, cn_value,
						kjqj_value, cbzx_value, zjzypzbh1_value, yssqh_value, fklx_value, gysbm_value, qt, sqrq_value,
						jbr_value, dgzfje_value, hbm_value, fkrq_value, "T", gysmc_value, gyskhh_value,
						gyskhzh_value, zjysm_value, "(付款)" + zy_value, fjzs_value,fkflhh_value,fkfyhzh_value);
				hEAD1.add(model1);
				JTCLFBXZJZFCreateModel head1 = new JTCLFBXZJZFCreateModel(hEAD1);
				try {
					xmlstring1 = XMLUtil.beanToXml(head1, JTCLFBXZJZFCreateModel.class);
				} catch (JAXBException e) {
					e.printStackTrace();
				}
				//gysgetforcw_yhxx_clf gysgetforcw_clf
				writeLog("股份公司-差旅费用报销-资金支付（对公）傳入xml参数：" + xmlstring1);
