 ...
 * 股份公司-对公预付款
 */
public class DGYFKFAction extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		String xmlstring = "";
		DGYFKSFMode model = new DGYFKSFMode(gsdm_value, zjpzbh_value, "Y", dszfje_value);
		try {
			xmlstring = XMLUtil.beanToXml(model, DGYFKSFMode.class);
			...
			if (null != map && !map.isEmpty()) {
				...
				if (!"E".equalsIgnoreCase(type)) {
					...
					setFailMessage(request, "failed",
							"股份公司-对公预付款失败：TYPE：" + type + " code：" + code + " message：" + message);
					return SUCCESS;
				}
			}
	...
	public void updateJTCLFBXSFDS(String requestid, String type, String code, String message, String bs) {
		...
		String sql = "update formtable_main_1105 set scode = '" + code + "', smessage = '" + message + "',stype = '"
