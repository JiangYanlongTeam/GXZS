package weaver.interfaces.gx.jyl.gfgs.dgyfk;
 ...
 * 股份公司-对公预付款
 */
public class DGYFKZYAction extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		if (!"submit".equals(src)) {
			new weaver.general.BaseBean().writeLog("股份公司-对公预付款，不执行接口.");
			return SUCCESS;
		}
		// 公司代码-值
		String gsdm_value = "";
		...
		writeLog("支付金额：" + zfje_value);
		writeLog("系统字段:" + xtzd_value);

		String xmlstring = "";
		DGYFKZYMode model = new DGYFKZYMode(gsdm_value, cn_value, kjqj_value, cbzx_value, yssqh_value,
				zfje_value, ysm_value, zy_value, "");
		try {
			xmlstring = XMLUtil.beanToXml(model, DGYFKZYMode.class);
		} catch (JAXBException e1) {
			e1.printStackTrace();
		}
		writeLog("股份公司-对公预付款传入xml参数：" + xmlstring);
		...
		try {
			...
			writeLog("股份公司-对公预付款返回消息：" + returnmessage);
			...
			if (null != map && !map.isEmpty()) {
				...
				if (!"E".equalsIgnoreCase(type)) {
					updateJTCLFFYBXZYDS(requestid, type, code, message, no, "");
				} else {
					setFailMessage(request, "failed", "股份公司-对公预付款失败：TYPE：" + type + " code：" + code
							+ " zjpzh：" + no + " message：" + message);
					return SUCCESS;
				}
			}
		} catch (RemoteException e) {
			setFailMessage(request, "failed", "股份公司-对公预付款接口异常：" + e);
			return SUCCESS;
		}
		return SUCCESS;
	}
     ...
     * @param message
     */
    public void updateJTCLFFYBXZYDS(String requestid,
                             String type,
                             String code,
                             String message,String no,String bs) {
        RecordSet rs = new RecordSet();
        String sql = "update formtable_main_1105 set fzjpzh = '"+no+"', fcode = '"+code+"', fmessage = '"+message+"',ftype = '"+type+"' where requestid = '"+requestid+"'";
        rs.execute(sql);
    }
