 ...
 * 股份公司-对公预付款资金支付单
 */
public class DGYFKZJZFCreateAction extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		String zhz_value = "";
		// 供应商编码
		String gysbm_column = "gysbm"; //TODO 没有
		String gysbm_value = "";
		// 付款方帐号
		...
		writeLog("股份公司-对公预付款傳入xml参数：" + xmlstring);
		...
		try {
			...
			if (null != map && !map.isEmpty()) {
				...
				if ("S".equalsIgnoreCase(type)) {
					updateJTCLFXBXZJZFDDG(requestid, type, code, message, no, "GFGS_CLFYBX");
				} else {
					setFailMessage(request, "failed", "股份公司-对公预付款失败：TYPE：" + type
     ...
     * @param message
     */
    public void updateJTCLFXBXZJZFDDG(String requestid,
                             String type,
                             String code,
                             String message,String no,String bs) {
        RecordSet rs = new RecordSet();
        String sql = "update formtable_main_1105 set capplyno = '"+no+"', ccode = '"+code+"', cmessage = '"+message+"',ctype = '"+type+"' where requestid = '"+requestid+"'";
        rs.execute(sql);
    }
}
