 ...
 * 股份公司-冲销对公预付款释放
 */
public class CXDGYFKSFAction extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		for (int i = 0; i < properties.length; i++) {
			...
			if (name.equals(dszfje_column)) {
				dszfje_value = Util.null2o(value);
			}
		}
		writeLog("系统字段：" + xtzd_value);
		writeLog("公司代码：" + gsdm_value);
		writeLog("资金凭证编号：" + zjpzbh_value);
		writeLog("发票金额：" + dszfje_value);

		if(Double.parseDouble(dszfje_value) <= 0) {
			return SUCCESS;
		}
		String xmlstring = "";
		CXDGYFKSFMode model = new CXDGYFKSFMode(gsdm_value, zjpzbh_value, "Y", dszfje_value);
		try {
			xmlstring = XMLUtil.beanToXml(model, CXDGYFKSFMode.class);
		} catch (JAXBException e) {
			e.printStackTrace();
		}

		writeLog("股份公司-冲销对公预付款释放传入xml参数：" + xmlstring);
		...
		try {
			...
			writeLog("股份公司-冲销对公预付款释放返回消息：" + returnmessage);
			...
			if (null != map && !map.isEmpty()) {
				...
				if (!"E".equalsIgnoreCase(type)) {
					updateJTCLFBXSFDS(requestid, type, code, message, "GFGS_CLFYBX");
				} else {
					setFailMessage(request, "failed",
							"股份公司-冲销对公预付款释放失败：TYPE：" + type + " code：" + code + " message：" + message);
					return SUCCESS;
				}
			}
		} catch (RemoteException e) {
			setFailMessage(request, "failed", "股份公司-冲销对公预付款释放接口异常：" + e);
			return SUCCESS;
		}
		return SUCCESS;
	}
	 ...
	 * @param message
	 */
	public void updateJTCLFBXSFDS(String requestid, String type, String code, String message, String bs) {
		RecordSet rs = new RecordSet();
		String sql = "update formtable_main_1106 set scode = '" + code + "', smessage = '" + message + "',stype = '"
