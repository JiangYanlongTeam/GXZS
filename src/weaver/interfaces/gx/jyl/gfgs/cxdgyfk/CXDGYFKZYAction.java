 ...
 * 股份公司-冲销对公预付款
 */
public class CXDGYFKZYAction extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		if (!"submit".equals(src)) {
			new weaver.general.BaseBean().writeLog("股份公司-冲销对公预付款，不执行接口.");
			return SUCCESS;
		}
		// 公司代码-值
		String gsdm_value = "";
		...
		writeLog("股份公司-冲销对公预付款传入xml参数：" + xmlstring);
		...
		try {
			...
			writeLog("股份公司-冲销对公预付款返回消息：" + returnmessage);
			...
			if (null != map && !map.isEmpty()) {
				...
				if (!"E".equalsIgnoreCase(type)) {
					...
					setFailMessage(request, "failed", "股份公司-冲销对公预付款失败：TYPE：" + type + " code：" + code
							+ " zjpzh：" + no + " message：" + message);
					return SUCCESS;
				}
			}
		} catch (RemoteException e) {
			setFailMessage(request, "failed", "股份公司-冲销对公预付款接口异常：" + e);
			return SUCCESS;
		}
		return SUCCESS;
	}
    ...
    public void updateJTCLFFYBXZYDS(String requestid,
        ...
        String sql = "update formtable_main_1106 set fzjpzh = '"+no+"', fcode = '"+code+"', fmessage = '"+message+"',ftype = '"+type+"' where requestid = '"+requestid+"'";
        rs.execute(sql);
    }
