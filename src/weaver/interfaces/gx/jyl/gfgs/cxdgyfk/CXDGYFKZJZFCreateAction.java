 ...
 * 股份公司-对公付款资金支付单
 */
public class CXDGYFKZJZFCreateAction extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		JTCLFBXZJZFCreate_HeadModel model = new JTCLFBXZJZFCreate_HeadModel(djlx_value, gsdm_value, cn_value,
				...
				jbr_value, df.format(cx), hbm_value, fkrq_value, "T", gysmc_value, gyskhh_value,
		...
		writeLog("股份公司-对公付款资金支付单傳入xml参数：" + xmlstring);
		...
		try {
			...
			writeLog("股份公司-对公付款资金支付单返回消息：" + returnmessage);
			...
			if (null != map && !map.isEmpty()) {
				...
				if ("S".equalsIgnoreCase(type)) {
					...
					setFailMessage(request, "failed", "股份公司-对公付款资金支付单失败：TYPE：" + type
							+ " code：" + code + " applyno：" + no + " message：" + message);
					return SUCCESS;
				}
			}
		} catch (RemoteException e) {
			setFailMessage(request, "failed", "股份公司-对公付款资金支付单接口异常：" + e);
			return SUCCESS;
		}
		return SUCCESS;
	}
    ...
    public void updateJTCLFXBXZJZFDDG(String requestid,
        ...
        String sql = "update formtable_main_1106 set capplyno = '"+no+"', ccode = '"+code+"', cmessage = '"+message+"',ctype = '"+type+"' where requestid = '"+requestid+"'";
        rs.execute(sql);
    }
}
