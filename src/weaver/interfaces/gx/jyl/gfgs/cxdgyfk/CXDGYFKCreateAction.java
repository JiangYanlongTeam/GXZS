 ...
 * 股份公司-冲销对公预付款创建凭证
 */
public class CXDGYFKCreateAction extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		if (!"submit".equals(src)) {
			new weaver.general.BaseBean().writeLog("股份公司-冲销对公预付款创建凭证退回操作，不执行接口.");
			return SUCCESS;
		}
		// 凭证类型-值
		String pzlx_value = "";
		...
		String tablename = "formtable_main_1106";
		...
		if (Double.parseDouble(fpje_value) == Double.parseDouble(cxje_value)) {
			...
			writeLog("股份公司-冲销对公预付款创建凭证传入xml参数：" + xmlstring);
			...
			try {
				...
				writeLog("股份公司-冲销对公预付款创建凭证返回消息：" + returnmessage);
				...
				if (null != map && !map.isEmpty()) {
					...
					if ("S".equalsIgnoreCase(ZSTATU)) {
						updateJTYBFBXCJ(requestid, RET_ACCNO, ZSTATU, MESSAGE, pzacdocno, "GFGS_YBFYBX");
					} else {
						publicmethod.setFailMessage(request, "failed", "股份公司-冲销对公预付款创建凭证失败：RET_ACCNO：" + RET_ACCNO
								+ " ZSTATU：" + ZSTATU + " ZDESC：" + MESSAGE);
						return SUCCESS;
					}
				}
			} catch (RemoteException e) {
				publicmethod.setFailMessage(request, "failed", "股份公司-冲销对公预付款创建凭证异常：" + e);
				return SUCCESS;
			}
		}
		...
		if (Double.parseDouble(fpje_value) < Double.parseDouble(cxje_value)) {
			...
			JTCLFBXCreate_ItemModel line0 = new JTCLFBXCreate_ItemModel("S", yhck_value, "", "", "",
					df.format(cx), "", "(挂帐)" + zy_value, "", ckpzbh_value, cbzx_value, "", "", ysm_value);
			lines.add(line0);
	...
	public String getDH(String requestid) {
		RecordSet rs = new RecordSet();
		String sql2 = "select sqbh from formtable_main_1105 where id in (select b.yfkdh from formtable_main_1106 a, formtable_main_1106_dt2 b where a.id = b.mainid and a.requestid = '"
				+ requestid + "')";
		rs.execute(sql2);
		rs.next();
    ...
    public void updateJTYBFBXCJ(String requestid,
        ...
        RecordSet rs = new RecordSet();
        String sql = "update formtable_main_1106 set pzaccno = '"+RET_ACCNO+"', pzstatus = '"+ZSTATU+"', pzmessage = '"+ZDESC+"',pzacdocno = '"+pzacdocno+"' where requestid = '"+requestid+"'";
        rs.execute(sql);
    }
