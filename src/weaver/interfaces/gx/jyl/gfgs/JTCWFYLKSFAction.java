...
import weaver.interfaces.gx.jyl.gfgs.model.JTCWFYJKSFModel;
 ...
 * 股份公司-费用借款-释放
 */
public class JTCWFYLKSFAction extends BaseBean implements Action {
    ...
    public String execute(RequestInfo request) {
        ...
        JTCWFYJKSFModel model = new JTCWFYJKSFModel(gsdm_value, zjpzbh_value, "Y", jkje_value);
		try {
			xmlstring = XMLUtil.beanToXml(model, JTCWFYJKSFModel.class);
		} catch (JAXBException e) {
			e.printStackTrace();
		}
		
        writeLog("股份公司-费用借款-释放传入xml参数：" + xmlstring);
        ...
        try {
            ...
            writeLog("股份公司-费用借款-释放返回消息：" + returnmessage);
            ...
            if (null != map && !map.isEmpty()) {
                ...
                if (!"E".equalsIgnoreCase(type)) {
                    publicmethod.updateJTCWFYJKSF(requestid, type, code, message, "GFGS_FYJK");
                } else {
                    publicmethod.setFailMessage(request, "failed", "股份公司-费用借款-释放失败：TYPE：" + type + " code：" + code + " message：" + message);
                    return SUCCESS;
                }
            }
        } catch (RemoteException e) {
            publicmethod.setFailMessage(request, "failed", "股份公司-费用借款-释放接口异常：" + e);
            return SUCCESS;
        }
        return SUCCESS;
    }
