...
import weaver.interfaces.gx.jyl.gfgs.model.JTYBFYBXSQSFModel;
 ...
 * 集团-一般费用报销释放
 */
public class JTYBFYBXSQSFAction extends BaseBean implements Action {
    ...
    public String execute(RequestInfo request) {
        ...
        String xmlstring = "";
        JTYBFYBXSQSFModel model = new JTYBFYBXSQSFModel(gsdm_value, zjpzbh_value, "Y", sbje_value);
		try {
			xmlstring = XMLUtil.beanToXml(model, JTYBFYBXSQSFModel.class);
		} catch (JAXBException e) {
			e.printStackTrace();
		}
    	
        writeLog("集团-一般费用报销释放传入xml参数：" + xmlstring);
        ...
        try {
            ...
            writeLog("集团-一般费用报销释放返回消息：" + returnmessage);
            ...
            if (null != map && !map.isEmpty()) {
                ...
                if (!"E".equalsIgnoreCase(type)) {
                    publicmethod.updateJTYBFYBXSF(requestid, type, code, message,"GFGS_YBFYBX");
                } else {
                    publicmethod.setFailMessage(request, "failed", "集团-一般费用报销释放失败：TYPE：" + type + " code：" + code + " message：" + message);
                    return SUCCESS;
                }
            }
        } catch (RemoteException e) {
            publicmethod.setFailMessage(request, "failed", "集团-一般费用报销释放接口异常：" + e);
            return SUCCESS;
        }
        return SUCCESS;
    }
