...
import weaver.interfaces.gx.jyl.gfgs.model.JTYBFYBXSQZYModel;
 ...
 * 股份公司-一般费用报销占用
 */
public class JTYBFYBXSQZYAction extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		if (!"submit".equals(src)) {
			new weaver.general.BaseBean().writeLog("股份公司-一般费用报销占用接口退回操作，不执行接口.");
			return SUCCESS;
		}
		// 公司代码-值
		String gsdm_value = "";
		...
		if (("2".equals(bxfs_value)) || ("0".equals(bxfs_value) && Double.parseDouble(sbje_value) > 0)) {
			String xmlstring = "";
			JTYBFYBXSQZYModel model = new JTYBFYBXSQZYModel(gsdm_value, cn_value, kjqj_value, cbzx_value, yssqh_value,
					sqje_value, ysm_value, zy_value, "");
			try {
				xmlstring = XMLUtil.beanToXml(model, JTYBFYBXSQZYModel.class);
			} catch (JAXBException e) {
				e.printStackTrace();
			}

			writeLog("股份公司-一般费用报销占用传入xml参数：" + xmlstring);
			...
			try {
				...
				writeLog("股份公司-一般费用报销占用返回消息：" + returnmessage);
				...
				if (null != map && !map.isEmpty()) {
					...
					if (!"E".equalsIgnoreCase(type)) {
						publicmethod.updateHJTYBFYBXZY(requestid, type, code, message, no,"GFGS_YBFYBX");
					} else {
						publicmethod.setFailMessage(request, "failed", "股份公司-一般费用报销占用失败：TYPE：" + type + " code：" + code
								+ " zjpzh：" + no + " message：" + message);
						return SUCCESS;
					}
				}
			} catch (RemoteException e) {
				publicmethod.setFailMessage(request, "failed", "股份公司-一般费用报销占用接口异常：" + e);
				return SUCCESS;
			}
		}
		return SUCCESS;
