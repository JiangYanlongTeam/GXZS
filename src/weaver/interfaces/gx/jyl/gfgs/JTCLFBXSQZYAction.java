...
import weaver.interfaces.gx.jyl.gfgs.model.JTCLFBXZYModel;
 ...
 * 股份公司-差旅费报销占用
 */
public class JTCLFBXSQZYAction extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		if (!"submit".equals(src)) {
			new weaver.general.BaseBean().writeLog("股份公司-差旅费报销占用接口退回操作，不执行接口.");
			return SUCCESS;
		}
		// 公司代码-值
		String gsdm_value = "";
		...
		if (("1".equals(bxfs_value)) || ("0".equals(bxfs_value) && Double.parseDouble(sbje_value) > 0)) {
			if (zflb_value.equals("1")) {// 对私
				String xmlstring = "";
				JTCLFBXZYModel model = new JTCLFBXZYModel(gsdm_value, cn_value, kjqj_value, cbzx_value, yssqh_value,
						dszfje_value, ysm_value, zy_value, "");
				try {
					xmlstring = XMLUtil.beanToXml(model, JTCLFBXZYModel.class);
				} catch (JAXBException e1) {
					e1.printStackTrace();
				}

				writeLog("股份公司-差旅费报销占用（对私）传入xml参数：" + xmlstring);
				...
				try {
					...
					writeLog("股份公司-差旅费报销占用（对私）返回消息：" + returnmessage);
					...
					if (null != map && !map.isEmpty()) {
						...
						if (!"E".equalsIgnoreCase(type)) {
							publicmethod.updateJTCLFFYBXZYDS(requestid, type, code, message, no, "GFGS_CLFYBX");
						} else {
							publicmethod.setFailMessage(request, "failed", "股份公司-差旅费报销占用（对私）失败：TYPE：" + type + " code："
									+ code + " zjpzh：" + no + " message：" + message);
							return SUCCESS;
						}
					}
				} catch (RemoteException e) {
					publicmethod.setFailMessage(request, "failed", "股份公司-差旅费报销占用（对私）接口异常：" + e);
					return SUCCESS;
				}
			}
			if (zflb_value.equals("2")) {// 对公
				String xmlstring = "";
				JTCLFBXZYModel model = new JTCLFBXZYModel(gsdm_value, cn_value, kjqj_value, cbzx_value, yssqh_value,
				...
				writeLog("股份公司-差旅费报销占用（对公）传入xml参数：" + xmlstring);
				...
				try {
					DT_1030_ALL2ERP_ZJYSZY_RETURN = proxy.SI_1030_ALL2ERP_ZJYSZY_OUT(DT_1030_ALL2ERP_ZJYSZY);
					String returnmessage = DT_1030_ALL2ERP_ZJYSZY_RETURN.getINPUT();
					writeLog("股份公司-差旅费报销占用（对公）返回消息：" + returnmessage);
					Map<String, String> map = publicmethod.readXMLForZY(returnmessage);
					if (null != map && !map.isEmpty()) {
						String type = (String) map.get("TYPE");
						String no = (String) map.get("ZJPZH");
						...
						String message = (String) map.get("MESSAGE");
						if (!"E".equalsIgnoreCase(type)) {
							publicmethod.updateJTCLFFYBXZYDG(requestid, type, code, message, no, "GFGS_CLFYBX");
						} else {
							publicmethod.setFailMessage(request, "failed", "股份公司-差旅费报销占用（对公）失败：TYPE：" + type + " code："
									+ code + " zjpzh：" + no + " message：" + message);
							return SUCCESS;
						}
					}
				} catch (RemoteException e) {
					publicmethod.setFailMessage(request, "failed", "股份公司-差旅费报销占用（对公）接口异常：" + e);
					return SUCCESS;
				}
			}
			if (zflb_value.equals("0")) {// 对私 对公
				...
				String xmlstring1 = "";
				JTCLFBXZYModel model1 = new JTCLFBXZYModel(gsdm_value, cn_value, kjqj_value, cbzx_value, yssqh_value,
						dgzfje_value, ysm_value, zy_value, "");
				try {
					xmlstring1 = XMLUtil.beanToXml(model1, JTCLFBXZYModel.class);
				} catch (JAXBException e1) {
					e1.printStackTrace();
				}

				writeLog("股份公司-差旅费报销占用（对公）传入xml参数：" + xmlstring1);
