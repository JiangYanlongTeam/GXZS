package weaver.interfaces.gx.jyl.gfgs.dgfk;
 ...
 * 股份公司-对公付款占用
 */
public class DGFKZYAction extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		if (!"submit".equals(src)) {
			new weaver.general.BaseBean().writeLog("股份公司-对公付款，不执行接口.");
			return SUCCESS;
		}
		// 公司代码-值
		String gsdm_value = "";
		...
		String xmlstring = "";
		DGFKZYMode model = new DGFKZYMode(gsdm_value, cn_value, kjqj_value, cbzx_value, yssqh_value,
				zfje_value, ysm_value, zy_value, "");
		try {
			xmlstring = XMLUtil.beanToXml(model, DGFKZYMode.class);
		} catch (JAXBException e1) {
			e1.printStackTrace();
		}
		writeLog("股份公司-对公付款传入xml参数：" + xmlstring);
		...
		try {
			...
			writeLog("股份公司-对公付款返回消息：" + returnmessage);
			...
			if (null != map && !map.isEmpty()) {
				...
				if (!"E".equalsIgnoreCase(type)) {
					...
					setFailMessage(request, "failed", "股份公司-对公付款失败：TYPE：" + type + " code：" + code
							+ " zjpzh：" + no + " message：" + message);
					return SUCCESS;
				}
			}
		} catch (RemoteException e) {
			setFailMessage(request, "failed", "股份公司-对公付款接口异常：" + e);
			return SUCCESS;
		}
		return SUCCESS;
	}
    ...
    public void updateJTCLFFYBXZYDS(String requestid,
        ...
        String sql = "update formtable_main_1104 set fzjpzh = '"+no+"', fcode = '"+code+"', fmessage = '"+message+"',ftype = '"+type+"' where requestid = '"+requestid+"'";
        rs.execute(sql);
    }
