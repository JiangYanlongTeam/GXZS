...
import weaver.interfaces.gx.jyl.jtfybx.model.JTFYBX_HKSQCreateModel;
import weaver.interfaces.gx.jyl.jtfybx.model.JTFYBX_HKSQCreate_HeadModel;
import weaver.interfaces.gx.jyl.jtfybx.model.JTFYBX_HKSQCreate_ItemModel;
 ...
 * 集团费用报销-还款申请-创建凭证
 */
public class JTFYBX_HKSQCreateAction extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		List<JTFYBX_HKSQCreate_HeadModel> headlist = new ArrayList<JTFYBX_HKSQCreate_HeadModel>();
		JTFYBX_HKSQCreate_HeadModel headermodel = new JTFYBX_HKSQCreate_HeadModel(gsdm_value, pzlx_value, pzrq_value, gzrq_value, 
				hksm_value, hkbh_value, "", "", pzzdr_value, "OA系统",fjzs_value);//TODO 附件张数
		headlist.add(headermodel);
		List<JTFYBX_HKSQCreate_ItemModel> lines = new ArrayList<JTFYBX_HKSQCreate_ItemModel>();
		JTFYBX_HKSQCreate_ItemModel line = new JTFYBX_HKSQCreate_ItemModel("S", xjyh_value, "", "", "", hkje_value, 
				"", zy_value, fjz_value, hkbh_value, cbzx_value, "", "", zjysm_value);
		lines.add(line);
		
		JTFYBX_HKSQCreate_ItemModel line2 = new JTFYBX_HKSQCreate_ItemModel("H", qtysk_value, ygbh_value, "", "", hkje_value, 
				"", zy_value, fjz_value, sqbh, "", "", "", zjysm_value);
		lines.add(line2);
		JTFYBX_HKSQCreateModel model = new JTFYBX_HKSQCreateModel(headlist,lines);
		try {
			xmlstring = XMLUtil.beanToXml(model, JTFYBX_HKSQCreateModel.class);
		...
		try {
			...
			if (null != map && !map.isEmpty()) {
                ...
                if ("S".equalsIgnoreCase(ZSTATU)) {
                	publicmethod.updateJTHKSQCJ(requestid, RET_ACCNO, ZSTATU,MESSAGE,pzacdocno,"JTFYBX_HKSQ");
                } else {
