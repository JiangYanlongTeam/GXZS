...
import weaver.interfaces.gx.jyl.jtfybx.model.JTFYBX_FYJKZYModel;
 ...
 * 集团费用-费用借款-预算占用
 */
public class JTFYBX_FYJKZYAction extends BaseBean implements Action {
    ...
    public String execute(RequestInfo request) {
        ...
        if (!"submit".equals(src)) {
            new weaver.general.BaseBean().writeLog("集团费用-费用借款-预算占用退回操作，不执行接口.");
            return SUCCESS;
        }
        //公司代码-值
        String gsdm_value = "";
        ...
        String xmlstring = "";
        JTFYBX_FYJKZYModel model = new JTFYBX_FYJKZYModel(gsdm_value, cn_value, kjqj_value, cbzx_value, yssqh_value, sqje_value, ysm_value, jkyt_value);
		try {
			xmlstring = XMLUtil.beanToXml(model, JTFYBX_FYJKZYModel.class);
		} catch (JAXBException e) {
			e.printStackTrace();
		}
		
        writeLog("集团费用-费用借款-预算占用传入xml参数：" + xmlstring);
        ...
        try {
            ...
            writeLog("集团费用-费用借款-预算占用返回消息：" + returnmessage);
            ...
            if (null != map && !map.isEmpty()) {
                ...
                if (!"E".equalsIgnoreCase(type)) {
                    publicmethod.updateJTZJYSZY(requestid, type, code, message,no,"JTFYBX_FYJK");
                } else {
                    publicmethod.setFailMessage(request, "failed", "集团费用-费用借款-预算占用失败：TYPE：" + type + " code：" + code + " zjpzh：" + no + " message：" + message);
                    return SUCCESS;
                }
            }
        } catch (RemoteException e) {
            publicmethod.setFailMessage(request, "failed", "集团费用-费用借款-预算占用接口异常：" + e);
            return SUCCESS;
        }
        return SUCCESS;
    }
