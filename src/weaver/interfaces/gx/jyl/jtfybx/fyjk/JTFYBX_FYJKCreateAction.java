...
import weaver.interfaces.gx.jyl.jtfybx.model.JTFYBX_FYJKCreateModel;
import weaver.interfaces.gx.jyl.jtfybx.model.JTFYBX_FYJKCreate_HeadModel;
 ...
 * 集团费用报销-费用借款-创建
 */
public class JTFYBX_FYJKCreateAction extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		if (!"submit".equals(src)) {
			new weaver.general.BaseBean().writeLog("集团费用报销-费用借款-创建退回操作，不执行接口.");
			return SUCCESS;
		}
		// 单据类型-值
		String djlx_value = "";
		...
		List<JTFYBX_FYJKCreate_HeadModel> hEAD = new ArrayList<JTFYBX_FYJKCreate_HeadModel>();
		JTFYBX_FYJKCreate_HeadModel model = new JTFYBX_FYJKCreate_HeadModel(djlx_value, gsdm_value, cn_value, kjqj_value, cbzx_value, yssqh_value,
		...
		hEAD.add(model);
		JTFYBX_FYJKCreateModel header = new JTFYBX_FYJKCreateModel(hEAD);
		try {
			xmlstring = XMLUtil.beanToXml(header, JTFYBX_FYJKCreateModel.class);
		} catch (JAXBException e) {
			e.printStackTrace();
		}
		
		writeLog("集团费用报销-费用借款-创建傳入xml参数：" + xmlstring);
		...
		try {
			...
			writeLog("集团费用报销-费用借款-创建返回消息：" + returnmessage);
			...
			if (null != map && !map.isEmpty()) {
				...
				if ("S".equalsIgnoreCase(type)) {
					publicmethod.updateJTFYJKSQ(requestid, type, code, message,
							no,"JTFYBX_FYJK");
				} else {
					publicmethod.setFailMessage(request, "failed",
							"集团费用报销-费用借款-创建失败：TYPE：" + type + " code：" + code
									+ " applyno：" + no + " message：" + message);
					return SUCCESS;
				}
			}
		} catch (RemoteException e) {
			publicmethod.setFailMessage(request, "failed", "集团费用报销-费用借款-创建接口异常："
					+ e);
			return SUCCESS;
		}
		return SUCCESS;
