...
import weaver.interfaces.gx.jyl.jtfybx.model.JTFYBX_CLFZJZFCreateModel;
import weaver.interfaces.gx.jyl.jtfybx.model.JTFYBX_CLFZJZFCreate_HeadModel;
 ...
 * 集团费用报销-差旅费用报销-资金支付
 */
public class JTFYBX_CLFZJZFCreateAction extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		if (!"submit".equals(src)) {
			new weaver.general.BaseBean().writeLog("集团费用报销-差旅费用报销-资金支付-创建退回操作，不执行接口.");
			return SUCCESS;
		}
		// 单据类型-值
		String djlx_value = "";
		...
		if (("1".equals(bxfs_value) || ("0".equals(bxfs_value) && Double.parseDouble(sqje_value) > 0))) {
			...
			if (zflb_value.equals("1")) {// 对私
				...
				writeLog("集团费用报销-差旅费用报销-资金支付（对私）傳入xml参数：" + xmlstring);
				...
				try {
					...
					writeLog("集团费用报销-差旅费用报销-资金支付（对私）返回消息：" + returnmessage);
					...
					if (null != map && !map.isEmpty()) {
						...
						if ("S".equalsIgnoreCase(type)) {
							publicmethod.updateJTCLFXBXZJZFDDS(requestid, type, code, message, no, "JTFYBX_CLFYBX");
						} else {
							publicmethod.setFailMessage(request, "failed", "集团费用报销-差旅费用报销-资金支付（对私）失败：TYPE：" + type
									+ " code：" + code + " applyno：" + no + " message：" + message);
							return SUCCESS;
						}
					}
				} catch (RemoteException e) {
					publicmethod.setFailMessage(request, "failed", "集团费用报销-差旅费用报销-资金支付（对私）接口异常：" + e);
					return SUCCESS;
				}
			}
			if (zflb_value.equals("2") || "4".equals(zflb_value)) {// 对公
				...
				writeLog("集团费用报销-差旅费用报销-资金支付（对公）傳入xml参数：" + xmlstring);
				SI_1049_ALL2ERP_ZJZFSQ_OUTProxy proxy = new SI_1049_ALL2ERP_ZJZFSQ_OUTProxy();
				DT_1049_ALL2ERP_ZJZFSQ DT_1049_ALL2ERP_ZJZFSQ = new DT_1049_ALL2ERP_ZJZFSQ();
				DT_1049_ALL2ERP_ZJZFSQ.setOUTPUT(xmlstring);
				DT_1049_ALL2ERP_ZJZFSQ.setSYSTEM(xtzd_value);
				...
				try {
					DT_1049_ALL2ERP_ZJZFSQ_RETURN = proxy.SI_1049_ALL2ERP_ZJZFSQ_OUT(DT_1049_ALL2ERP_ZJZFSQ);
					String returnmessage = DT_1049_ALL2ERP_ZJZFSQ_RETURN.getINPUT();
					writeLog("集团费用报销-差旅费用报销-资金支付（对公）返回消息：" + returnmessage);
					Map<String, String> map = publicmethod.readXMLForSQ(returnmessage);
					if (null != map && !map.isEmpty()) {
						String type = (String) map.get("TYPE");
						String no = (String) map.get("APPLYNO");
						...
						String message = (String) map.get("MESSAGE");
						if ("S".equalsIgnoreCase(type)) {
							publicmethod.updateJTCLFXBXZJZFDDG(requestid, type, code, message, no, "JTFYBX_CLFYBX");
						} else {
							publicmethod.setFailMessage(request, "failed", "集团费用报销-差旅费用报销-资金支付（对公）失败：TYPE：" + type
									+ " code：" + code + " applyno：" + no + " message：" + message);
							return SUCCESS;
						}
					}
				} catch (RemoteException e) {
					publicmethod.setFailMessage(request, "failed", "集团费用报销-差旅费用报销-资金支付（对公）接口异常：" + e);
					return SUCCESS;
				}
			}
			if (zflb_value.equals("0")) {// 对私 对公
				...
				writeLog("集团费用报销-差旅费用报销-资金支付（对公）傳入xml参数：" + xmlstring1);
