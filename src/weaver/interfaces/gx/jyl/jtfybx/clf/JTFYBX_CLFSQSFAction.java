...
import weaver.interfaces.gx.jyl.jtfybx.model.JTFYBX_CLFSFModel;
 ...
 * 集团费用报销-差旅费报销释放
 */
public class JTFYBX_CLFSQSFAction extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		if (zflb_value.equals("1")) {// 对私
			...
			writeLog("集团费用报销-差旅费报销释放（对私）传入xml参数：" + xmlstring);
			...
			try {
				...
				writeLog("集团费用报销-差旅费报销释放（对私）返回消息：" + returnmessage);
				...
				if (null != map && !map.isEmpty()) {
					...
					if (!"E".equalsIgnoreCase(type)) {
						publicmethod.updateJTCLFBXSFDS(requestid, type, code, message, "JTFYBX_CLFYBX");
					} else {
						publicmethod.setFailMessage(request, "failed",
								"集团费用报销-差旅费报销释放（对私）失败：TYPE：" + type + " code：" + code + " message：" + message);
						return SUCCESS;
					}
				}
			} catch (RemoteException e) {
				publicmethod.setFailMessage(request, "failed", "集团费用报销-差旅费报销释放（对私）接口异常：" + e);
				return SUCCESS;
			}
		}
		if (zflb_value.equals("2") || zflb_value.equals("4")) {// 对公
			...
			writeLog("集团费用报销-差旅费报销释放（对公）传入xml参数：" + xmlstring);
			...
			try {
				DT_1029_ALL2ERP_ZJYSSF_RETURN = proxy.SI_1029_ALL2ERP_ZJYSSF_OUT(DT_1029_ALL2ERP_ZJYSSF);
				String returnmessage = DT_1029_ALL2ERP_ZJYSSF_RETURN.getINPUT();
				writeLog("集团费用报销-差旅费报销释放（对公）返回消息：" + returnmessage);
				Map<String, String> map = publicmethod.readXMLForSF(returnmessage);
				if (null != map && !map.isEmpty()) {
					String type = (String) map.get("TYPE");
					String code = (String) map.get("CODE");
					String message = (String) map.get("MESSAGE");
					if (!"E".equalsIgnoreCase(type)) {
						publicmethod.updateJTCLFBXSFDG(requestid, type, code, message, "JTFYBX_CLFYBX");
					} else {
						publicmethod.setFailMessage(request, "failed",
								"集团费用报销-差旅费报销释放（对公）失败：TYPE：" + type + " code：" + code + " message：" + message);
						return SUCCESS;
					}
				}
			} catch (RemoteException e) {
				publicmethod.setFailMessage(request, "failed", "集团费用报销-差旅费报销释放（对公）接口异常：" + e);
				return SUCCESS;
			}
		}
		if (zflb_value.equals("0")) {// 对私 对公
			...
			writeLog("集团费用报销-差旅费报销释放（对公）传入xml参数：" + xmlstring1);
