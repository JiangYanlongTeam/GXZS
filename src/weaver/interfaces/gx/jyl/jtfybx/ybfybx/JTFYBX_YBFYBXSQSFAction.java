...
import weaver.interfaces.gx.jyl.jtfybx.model.JTFYBX_YBFYBXSQSFModel;
 ...
 * 集团费用报销-一般费用报销释放
 */
public class JTFYBX_YBFYBXSQSFAction extends BaseBean implements Action {
    ...
    public String execute(RequestInfo request) {
        ...
        writeLog("集团费用报销-一般费用报销释放传入xml参数：" + xmlstring);
        ...
        try {
            ...
            writeLog("集团费用报销-一般费用报销释放返回消息：" + returnmessage);
            ...
            if (null != map && !map.isEmpty()) {
                ...
                if (!"E".equalsIgnoreCase(type)) {
                    publicmethod.updateJTYBFYBXSF(requestid, type, code, message,"JTFYBX_YBFYBX");
                } else {
                    publicmethod.setFailMessage(request, "failed", "集团费用报销-一般费用报销释放失败：TYPE：" + type + " code：" + code + " message：" + message);
                    return SUCCESS;
                }
            }
        } catch (RemoteException e) {
            publicmethod.setFailMessage(request, "failed", "集团费用报销-一般费用报销释放接口异常：" + e);
            return SUCCESS;
        }
        return SUCCESS;
    }
