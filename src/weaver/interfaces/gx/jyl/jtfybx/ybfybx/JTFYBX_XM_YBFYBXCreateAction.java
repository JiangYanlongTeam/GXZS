...
import weaver.interfaces.gx.jyl.jtfybx.model.JTFYBX_YBFYBXCreateModel;
import weaver.interfaces.gx.jyl.jtfybx.model.JTFYBX_YBFYBXCreate_HeadModel;
import weaver.interfaces.gx.jyl.jtfybx.model.JTFYBX_YBFYBXCreate_ItemModel;
 ...
 * 集团费用报销-一般费用报销
 */
public class JTFYBX_XM_YBFYBXCreateAction extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		String tablename = getPropValue("GXBX", "JTFYBX_YBFYBX");
		String splx = getPropValue("GXBX", "JTFYBX_YBFYBX_FYSPLX");
		...
		if ("2".equals(bxfs_value)) {
			...
			if ("0".equals(zflb_value)) {
				...
				String sql = "select b.yskm,b.jxskm,b.bxje,b.se,sfzp,b.sl,a.cbzx,b.cph,b.jtwbs from " + tablename + " a," + tablename
				...
				writeLog("集团费用报销-一般费用报销(直接报销／个人)传入xml参数：" + xmlstring);
				...
				try {
					...
					writeLog("集团费用报销-一般费用报销(直接报销／个人)创建返回消息：" + returnmessage);
					...
					if (null != map && !map.isEmpty()) {
						...
						if ("S".equalsIgnoreCase(ZSTATU)) {
							publicmethod.updateJTYBFBXCJ(requestid, RET_ACCNO, ZSTATU, MESSAGE, pzacdocno, "JTFYBX_YBFYBX");
						} else {
							publicmethod.setFailMessage(request, "failed",
									"集团费用报销-一般费用报销(直接报销／个人)创建失败：RET_ACCNO：" + RET_ACCNO + " ZSTATU：" + ZSTATU + " ZDESC：" + MESSAGE);
							return SUCCESS;
						}
					}
				} catch (RemoteException e) {
					publicmethod.setFailMessage(request, "failed", "集团费用报销-一般费用报销(直接报销／个人)创建接口异常：" + e);
					return SUCCESS;
				}
			}
			//  对公
			if ("1".equals(zflb_value) || "4".equals(zflb_value)) {
				...
				writeLog("集团费用报销-一般费用报销(直接报销／对公)传入xml参数：" + xmlstring);
				...
				try {
					DT_1072_ALL2ERP_KJPZ_RETURN = proxy.SI_1072_ALL2ERP_KJPZ_OUT(DT_1072_ALL2ERP_KJPZ);
					String returnmessage = DT_1072_ALL2ERP_KJPZ_RETURN.getINPUT();
					writeLog("集团费用报销-一般费用报销(直接报销／对公)创建返回消息：" + returnmessage);
					Map<String, String> map = publicmethod.readXMLForCLFBXSQAA(returnmessage);
					if (null != map && !map.isEmpty()) {
						String RET_ACCNO = (String) map.get("RET_ACCNO");
						String ZSTATU = (String) map.get("MTYPE");
						...
						if ("S".equalsIgnoreCase(ZSTATU)) {
							...
							publicmethod.setFailMessage(request, "failed",
									"集团费用报销-一般费用报销(直接报销／对公)创建失败：RET_ACCNO：" + RET_ACCNO + " ZSTATU：" + ZSTATU + " ZDESC：" + MESSAGE);
							return SUCCESS;
						}
					}
				} catch (RemoteException e) {
					publicmethod.setFailMessage(request, "failed", "集团费用报销-一般费用报销(直接报销／对公)创建接口异常：" + e);
					return SUCCESS;
				}
			}
		}
		...
		if ("0".equals(bxfs_value) && "0".equals(sbje_value)) {
			...
			writeLog("集团费用报销-一般费用报销(冲销借款/个人 实报金额等于0)传入xml参数：" + xmlstring);
			...
			try {
				DT_1072_ALL2ERP_KJPZ_RETURN = proxy.SI_1072_ALL2ERP_KJPZ_OUT(DT_1072_ALL2ERP_KJPZ);
				String returnmessage = DT_1072_ALL2ERP_KJPZ_RETURN.getINPUT();
				writeLog("集团费用报销-一般费用报销(冲销借款/个人 实报金额等于0)创建返回消息：" + returnmessage);
				Map<String, String> map = publicmethod.readXMLForCLFBXSQAA(returnmessage);
				if (null != map && !map.isEmpty()) {
					String RET_ACCNO = (String) map.get("RET_ACCNO");
					String ZSTATU = (String) map.get("MTYPE");
					...
					if ("S".equalsIgnoreCase(ZSTATU)) {
						...
						publicmethod.setFailMessage(request, "failed",
								"集团费用报销-一般费用报销(冲销借款/个人)创建失败：RET_ACCNO：" + RET_ACCNO + " ZSTATU：" + ZSTATU + " ZDESC：" + MESSAGE);
						return SUCCESS;
					}
				}
			} catch (RemoteException e) {
				publicmethod.setFailMessage(request, "failed", "集团费用报销-一般费用报销(冲销借款/个人)创建接口异常：" + e);
				return SUCCESS;
			}
		}
		if ("0".equals(bxfs_value) && Double.parseDouble(sbje_value) > 0) {
			...
			writeLog("集团费用报销-一般费用报销(冲销借款／个人 实报金额大于0)传入xml参数：" + xmlstring);
			...
			try {
				DT_1072_ALL2ERP_KJPZ_RETURN = proxy.SI_1072_ALL2ERP_KJPZ_OUT(DT_1072_ALL2ERP_KJPZ);
				String returnmessage = DT_1072_ALL2ERP_KJPZ_RETURN.getINPUT();
				writeLog("集团费用报销-一般费用报销(冲销借款／个人 实报金额大于0)创建返回消息：" + returnmessage);
				Map<String, String> map = publicmethod.readXMLForCLFBXSQAA(returnmessage);
				if (null != map && !map.isEmpty()) {
					String RET_ACCNO = (String) map.get("RET_ACCNO");
					String ZSTATU = (String) map.get("MTYPE");
