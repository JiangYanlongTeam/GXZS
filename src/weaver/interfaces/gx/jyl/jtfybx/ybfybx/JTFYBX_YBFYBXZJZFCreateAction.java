...
import weaver.interfaces.gx.jyl.jtfybx.model.JTFYBX_YBFYBXZJZFCreateModel;
import weaver.interfaces.gx.jyl.jtfybx.model.JTFYBX_YBFYBXZJZFCreate_HeadModel;
 ...
 * 集团费用报销-通用费用报销-资金支付
 */
public class JTFYBX_YBFYBXZJZFCreateAction extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		if (!"submit".equals(src)) {
			new weaver.general.BaseBean().writeLog("集团费用报销-通用费用报销-资金支付-创建退回操作，不执行接口.");
			return SUCCESS;
		}
		// 单据类型-值
		String djlx_value = "";
		...
		if (("2".equals(bxfs_value) || ("0".equals(bxfs_value) && Double.parseDouble(sqje_value) > 0))) {
			...
			writeLog("集团费用报销-通用费用报销-资金支付傳入xml参数：" + xmlstring);
			...
			try {
				...
				writeLog("集团费用报销-通用费用报销-资金支付返回消息：" + returnmessage);
				...
				if (null != map && !map.isEmpty()) {
					...
					if ("S".equalsIgnoreCase(type)) {
						publicmethod.updateJTYBFYBXZJZFD(requestid, type, code, message, no, "JTFYBX_YBFYBX");
					} else {
						publicmethod.setFailMessage(request, "failed", "集团费用报销-通用费用报销-资金支付失败：TYPE：" + type + " code："
								+ code + " applyno：" + no + " message：" + message);
						return SUCCESS;
					}
				}
			} catch (RemoteException e) {
				publicmethod.setFailMessage(request, "failed", "集团费用报销-通用费用报销-资金支付接口异常：" + e);
				return SUCCESS;
			}
		}
		return SUCCESS;
