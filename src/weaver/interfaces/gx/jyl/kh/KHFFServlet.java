 ...
 * 客户分发
 */
public class KHFFServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) {
        ...
        List<FFXTMode> list = PublicMethods.getFFXT(gsdm, "客户");
        if("".equals(jjzxdms)){
            JSONObject jsonresult = new JSONObject();
            jsonresult.put("flag", "0");
        ...
        CustomersUpdate[] updates = new CustomersUpdate[dms.length];
        for(int m = 0; m < dms.length; m++) {
            ...
            try {
                fres  = f.searchCustomers(query, publicmethod.setReposInfo());
            } catch (RemoteException e1) {
                e1.printStackTrace();
            }
            if(null == fres || fres.getCustomers().length <= 0){
                JSONObject jsonresult = new JSONObject();
                jsonresult.put("flag", "0");
        ...
        try {
            ExecutionStatus[] executionStatus = f.updateCustomers(updates, publicmethod.setReposInfo(), updateConfigurationParameters);
