 ...
 * 股份公司-固定资产释放
 */
public class GDZCSFAction extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		writeLog("金额：" + dszfje_value);

		String xmlstring = "";
		GDZCSFMode model = new GDZCSFMode(gsdm_value, zjpzbh_value, "Y", dszfje_value);
		try {
			xmlstring = XMLUtil.beanToXml(model, GDZCSFMode.class);
		} catch (JAXBException e) {
			e.printStackTrace();
		}

		writeLog("股份公司-固定资产资金释放传入xml参数：" + xmlstring);
		...
		try {
			...
			writeLog("股份公司-固定资产资金释放返回消息：" + returnmessage);
			...
			if (null != map && !map.isEmpty()) {
				...
				if (!"E".equalsIgnoreCase(type)) {
					...
					setFailMessage(request, "failed",
							"股份公司-固定资产资金释放失败：TYPE：" + type + " code：" + code + " message：" + message);
					return SUCCESS;
				}
			}
		} catch (RemoteException e) {
			setFailMessage(request, "failed", "股份公司-固定资产资金释放接口异常：" + e);
			return SUCCESS;
		}
		return SUCCESS;
	}
	...
	public void updateJTCLFBXSFDS(String requestid, String type, String code, String message, String bs) {
		...
		String sql = "update formtable_main_864 set scode = '" + code + "', smessage = '" + message + "',stype = '"
