 ...
 * 股份公司-固定资产占用
 */
public class GDZCZYAction extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		if (!"submit".equals(src)) {
			new weaver.general.BaseBean().writeLog("股份公司-固定资产占用退回，不执行接口.");
			return SUCCESS;
		}
		...
		writeLog("股份公司-固定资产资金占用传入xml参数：" + xmlstring);
		...
		try {
			...
			writeLog("股份公司-固定资产资金占用返回消息：" + returnmessage);
			...
			if (null != map && !map.isEmpty()) {
				...
				if (!"E".equalsIgnoreCase(type)) {
					...
					setFailMessage(request, "failed",
							"股份公司-固定资产资金占用失败：TYPE：" + type + " code：" + code + " zjpzh：" + no + " message：" + message);
					return SUCCESS;
				}
			}
		} catch (RemoteException e) {
			setFailMessage(request, "failed", "股份公司-固定资产资金占用接口异常：" + e);
			return SUCCESS;
		}
	 ...
	 * @param message
	 */
	public void updateJTCLFFYBXZYDS(String requestid, String type, String code, String message, String no, String bs) {
		RecordSet rs = new RecordSet();
		String sql = "update formtable_main_864 set fzjpzh = '" + no + "', fcode = '" + code + "', fmessage = '"
