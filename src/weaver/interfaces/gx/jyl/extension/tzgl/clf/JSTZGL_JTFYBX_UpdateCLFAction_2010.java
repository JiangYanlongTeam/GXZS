...
public class JSTZGL_JTFYBX_UpdateCLFAction_2010 extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		if(zflb_value.equals("1")) {
			...
			if ("0".equals(dgzffs_value)) {
				String sql = "update " + table + " set skrkhh = '' ,skrlhh = '', skrzh = '', gysmc = '', dgzfje = '0' lhh = '', khh = '', gysyhzh = '' "
			...
			if ("1".equals(dgzffs_value)) {
				String sql = "update " + table + " set gysmc = '', dgzfje = '0' lhh = '', khh = '', gysyhzh = '' "
						+ "where requestid = '"+requestid+"'";
				writeLog("差旅费，支付类别为对私(现金)，清空个人收款信息／对公信息SQL：" + sql);
				rs.execute(sql);
			}
		...
		if(zflb_value.equals("2") || zflb_value.equals("4")) {
			String sql = "update " + table + " set " + dgzffs_column + " = ''," + skr_column + " = '',"+ dszfje_column + " = 0,skrlhh = '', skrzh = '' where requestid = '"+requestid+"'";
			writeLog("差旅费，支付类别为对公，清空对私信息SQL：" + sql);
			rs.execute(sql);
		}
		//不需要支付
		if(zflb_value.equals("3")) {
			String sql = "update " + table + " set " + dgzffs_column + " = ''," + gysmc_column + " = ''," +lhh_column + " = ''," + khh_column + " = '',"
					+ gysyhzh_column + " = ''," + dgzfje_column + " = 0,"+ skr_column + " = '',"+ dszfje_column + " = 0 ,skrlhh = '', skrzh = ''  where requestid = '"+requestid+"'";
