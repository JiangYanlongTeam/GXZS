...
import weaver.interfaces.gx.jyl.extension.tzgl.Mode.DGFKZYMode;
...
public class JSTZGL_DGFKZYAction_2010 extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		if (!"submit".equals(src)) {
			new BaseBean().writeLog("对公付款，不执行接口.");
			return SUCCESS;
		}
		// 公司代码-值
		String gsdm_value = "";
		...
		try {
			...
			if (null != map && !map.isEmpty()) {
				...
				if (!"E".equalsIgnoreCase(type)) {
					...
					setFailMessage(request, "failed", "对公付款失败：TYPE：" + type + " code：" + code
