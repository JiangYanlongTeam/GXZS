package weaver.interfaces.gx.jyl.extension.tzgl.dgyfk;
...
import weaver.interfaces.gx.jyl.extension.tzgl.Mode.DGYFKZYMode;
...
public class JSTZGL_DGYFKZYAction_2010 extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		if (!"submit".equals(src)) {
			new BaseBean().writeLog("对公预付款，不执行接口.");
			return SUCCESS;
		}
		// 公司代码-值
		String gsdm_value = "";
		...
		writeLog("对公预付款传入xml参数：" + xmlstring);
		...
		try {
			...
			writeLog("对公预付款返回消息：" + returnmessage);
			...
			if (null != map && !map.isEmpty()) {
				...
				if (!"E".equalsIgnoreCase(type)) {
					...
					setFailMessage(request, "failed", "对公预付款失败：TYPE：" + type + " code：" + code
							+ " zjpzh：" + no + " message：" + message);
					return SUCCESS;
				}
			}
		} catch (RemoteException e) {
			setFailMessage(request, "failed", "对公预付款接口异常：" + e);
			return SUCCESS;
		}
		return SUCCESS;
	}
