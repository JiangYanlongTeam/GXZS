...
public class JSTZGL_DGYFKZJZFCreateAction_2010 extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		if (!"submit".equals(src)) {
			new BaseBean().writeLog("对公预付款资金支付单退回操作，不执行接口.");
			return SUCCESS;
		}
		// 单据类型-值
		String djlx_value = "";
		...
		writeLog("对公预付款傳入xml参数：" + xmlstring);
		...
		try {
			...
			if (null != map && !map.isEmpty()) {
				...
				if ("S".equalsIgnoreCase(type)) {
					...
					setFailMessage(request, "failed", "对公预付款失败：TYPE：" + type
