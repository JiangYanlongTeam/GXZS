package weaver.interfaces.gx.jyl.extension.yygs.gdzc;
...
import weaver.interfaces.gx.jyl.extension.tzgl.Mode.GDZCSFMode;
...
public class GDZCSFAction_7180 extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		writeLog("固定资产资金释放传入xml参数：" + xmlstring);
		...
		try {
			...
			writeLog("固定资产资金释放返回消息：" + returnmessage);
			...
			if (null != map && !map.isEmpty()) {
				...
				if (!"E".equalsIgnoreCase(type)) {
					...
					setFailMessage(request, "failed",
							"固定资产资金释放失败：TYPE：" + type + " code：" + code + " message：" + message);
					return SUCCESS;
				}
			}
		} catch (RemoteException e) {
			setFailMessage(request, "failed", "固定资产资金释放接口异常：" + e);
			return SUCCESS;
		}
		return SUCCESS;
	}
