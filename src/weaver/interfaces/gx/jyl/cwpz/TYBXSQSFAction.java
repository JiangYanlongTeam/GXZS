 ...
 * 通用费用报销释放
 */
public class TYBXSQSFAction extends BaseBean implements Action {
    ...
    public String execute(RequestInfo request) {
        ...
        writeLog("通用费用报销释放传入xml参数：" + xmlstring);
        ...
        try {
            ...
            writeLog("通用费用报销释放放返回消息：" + returnmessage);
            ...
            if (null != map && !map.isEmpty()) {
                ...
                if ("S".equalsIgnoreCase(type)) {
                    publicmethod.updateTYSF(requestid, type, code, message);
                } else {
                    publicmethod.setFailMessage(request, "failed", "通用费用报销释放放失败：TYPE：" + type + " code：" + code + " message：" + message);
                    return SUCCESS;
                }
            }
        } catch (RemoteException e) {
            publicmethod.setFailMessage(request, "failed", "通用费用报销释放放接口异常：" + e);
            return SUCCESS;
        }
        return SUCCESS;
    }
