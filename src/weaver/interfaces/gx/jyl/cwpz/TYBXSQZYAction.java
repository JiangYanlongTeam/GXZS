 ...
 * 通用费用报销占用
 */
public class TYBXSQZYAction extends BaseBean implements Action {
    ...
    public String execute(RequestInfo request) {
        ...
        if (!"submit".equals(src)) {
            new weaver.general.BaseBean().writeLog("通用费用报销占用接口退回操作，不执行接口.");
            return SUCCESS;
        }
        //公司代码-值
        String gsdm_value = "";
        ...
        if(("2".equals(sfcxjk_value)) || ("0".equals(sfcxjk_value) && Double.parseDouble(sbje_value) > 0) || ("1".equals(sfcxjk_value) && Double.parseDouble(sbje_value) > 0)){
            ...
            writeLog("通用费用报销占用传入xml参数：" + xmlstring);
            ...
            try {
                ...
                writeLog("通用费用报销占用返回消息：" + returnmessage);
                ...
                if (null != map && !map.isEmpty()) {
                    ...
                    if ("S".equalsIgnoreCase(type) || "W".equalsIgnoreCase(type)) {
                        publicmethod.updateTYZY(requestid, type, code, message,no);
                    } else {
                        publicmethod.setFailMessage(request, "failed", "通用费用报销占用失败：TYPE：" + type + " code：" + code + " zjpzh：" + no + " message：" + message);
                        return SUCCESS;
                    }
                }
            } catch (RemoteException e) {
                publicmethod.setFailMessage(request, "failed", "通用费用报销占用接口异常：" + e);
                return SUCCESS;
            }
        }
        return SUCCESS;
