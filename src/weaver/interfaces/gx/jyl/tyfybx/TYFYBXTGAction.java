...
/**
 * 通用费用报销： 审批通过将在途借款金额释放，同时将在途借款金额变为累计借款金额
 */
public class TYFYBXTGAction extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
        ...
        writeLog("审批通过将在途借款金额释放，同时将在途借款金额变为累计借款金额 总金额："+ total);
        if("0".equals(bxfs_value)) {
        	String tablename = getPropValue("GXBX", "CW_FYJK");
        	String sql = "update "+tablename+" set hkztje=hkztje-'"+total+"',ljhkje=ljhkje+'"+total+"' where id='"+jkdh_value+"'  ";
        	writeLog("审批通过将在途借款金额释放，同时将在途借款金额变为累计借款金额: " + sql);
        	rs.execute(sql);
        }
        if("1".equals(bxfs_value)) {
        	String tablename = getPropValue("GXBX", "CW_YFKB");
        	String sql = "update "+tablename+" set ztcyfk=ztcyfk-'"+dou2+"',ljcyfk=ljcyfk+'"+dou2+"' where id='"+zjhbdh_value+"'  ";
