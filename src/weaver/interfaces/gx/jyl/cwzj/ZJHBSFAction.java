 ...
 * 资金划拨释放
 */
public class ZJHBSFAction extends BaseBean implements Action {
    ...
    public String execute(RequestInfo request) {
      ...
      //创建根节点ROOT
        ...
        writeLog("资金划拨释放传入xml参数：" + xmlstring);
        ...
        try {
            ...
            writeLog("资金划拨释放返回消息：" + returnmessage);
            ...
            if (null != map && !map.isEmpty()) {
                ...
                if ("S".equalsIgnoreCase(type)) {
                    publicmethod.updateZJHBSF(requestid, type, code, message);
                } else {
                    publicmethod.setFailMessage(request, "failed", "调用资金划拨释放失败：TYPE：" + type + " code：" + code + " message：" + message);
                    return SUCCESS;
                }
            }
        } catch (RemoteException e) {
            publicmethod.setFailMessage(request, "failed", "调用资金划拨释放接口异常：" + e);
            return SUCCESS;
        }
        return SUCCESS;
    }
