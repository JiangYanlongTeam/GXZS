 ...
 * 资金划拨支付申请创建
 */
public class ZJHBSQCreateAction extends BaseBean implements Action {
    ...
    public String execute(RequestInfo request) {
        ...
        if (!"submit".equals(src)) {
            new weaver.general.BaseBean().writeLog("资金划拨单退回操作，不执行接口.");
            return SUCCESS;
        }
        //单据类型-值
        String djlx_value = "";
        ...
        String skfhm_value = "";
        //收款方户名-字段（收款单位名称）
        String skfhm_column = "skdwmc";
        //收款方开户行CNAPS号-值
        String skfkhh_value = "";
        //收款方开户行CNAPS号-字段
        String skfkhh_column = "khyh";
        //收款方账号-值
        String skfzh_value = "";
        //收款方账号-字段
    	...
    	writeLog("资金划拨支付申请创建傳入xml参数："+xmlstring);
        ...
        try {
            ...
            writeLog("资金划拨创建返回消息：" + returnmessage);
            ...
            if (null != map && !map.isEmpty()) {
                ...
                if ("S".equalsIgnoreCase(type)) {
                    publicmethod.updateZJHBSQ(requestid, type, code, message, no);
                } else {
                    publicmethod.setFailMessage(request, "failed", "调用资金划拨申请单创建失败：TYPE：" + type + " code：" + code + " applyno：" + no + " message：" + message);
                    return SUCCESS;
                }
            }
        } catch (RemoteException e) {
            publicmethod.setFailMessage(request, "failed", "调用资金划拨申请单创建接口异常：" + e);
            return SUCCESS;
        }
        return SUCCESS;
    }
