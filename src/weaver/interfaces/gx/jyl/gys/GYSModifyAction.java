...
public class GYSModifyAction extends BaseBean implements Action {
	...
	public String execute(RequestInfo request) {
		...
		ServiceRelationshipsTupleTuple[] serviceRelationshipsTupleTuples = new ServiceRelationshipsTupleTuple[list
				.size()];
		for (int i = 0; i < list.size(); i++) {
			...
			ServiceRelationshipsTupleTuple[] ServiceRelationshipsTupleTuples = vendorsResultSet.getVendors()[0]
					.getServiceRelationshipsTuple();
		...
		try {
			executionStatus = proxy.updateVendors(vendorsCreates, publicmethod.setReposInfo(),
		...
		if (null != executionStatus && executionStatus.length > 0) {
			...
			if (!"OK".equalsIgnoreCase(status)) {
				PublicMethods.setFailedMessage(request, "failed",
						"连接MDM接口失败:" + executionStatus[0].getFault()[0].getText());
			} else {
